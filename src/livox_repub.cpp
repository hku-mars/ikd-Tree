...
void LivoxMsgCbk1(const livox_ros_driver::CustomMsgConstPtr& livox_msg_in) {
  ...
  for (size_t j = 0; j < livox_data.size(); j++) {
    ...
    for (unsigned int i = 0; i < livox_msg->point_num; ++i) {
      ...
      float offset_time_ms = livox_msg->points[i].offset_time / float(1000000);

      pt.intensity = livox_msg->points[i].line +livox_msg->points[i].reflectivity /10000.0 ; // The integer part is line number and the decimal part is reflectivity
      pt.curvature = offset_time_ms; // curvature is the time_offset in usecond.
      pcl_in.push_back(pt);
    }
  }
